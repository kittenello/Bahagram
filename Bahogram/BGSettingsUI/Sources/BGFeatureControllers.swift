...
func bgAppearanceSettingsController(context: AccountContext) -> ViewController {
    ...
    return bgController(context: context, title: "Оформление", entries: {
        [.header(0, 0, "ВКЛАДКИ"), .toggle(1, 0, "hideTabBar", "Скрыть панель вкладок", s.hideTabBar, true), .toggle(2, 0, "contacts", "Вкладка Контакты", s.showContactsTab, !s.hideTabBar), .toggle(3, 0, "calls", "Вкладка Звонки", s.showCallsTab, !s.hideTabBar), .toggle(4, 0, "wideTabBar", "Широкая Панель", s.wideTabBar, !s.hideTabBar), .header(10, 1, "ПРОФИЛИ"), .toggle(11, 1, "profileId", "ID Профилей", s.showProfileId, true), .toggle(12, 1, "dc", "Показывать дата-центр (DC)", s.showDc, true), .toggle(13, 1, "regDate", "Показывать дату регистрации", s.showRegistrationDate, true), .toggle(14, 1, "chatDate", "Показывать дату создания чата", s.showChatCreationDate, true), .toggle(15, 1, "confirmCalls", "Подтверждение вызова", s.confirmCalls, true), .header(20, 2, "ДРУГОЕ"), .toggle(21, 2, "disableAds", "Отключить рекламу", s.disableAds, true)]
    }, toggle: { key, value in
        switch key { case "hideTabBar": s.hideTabBar = value; case "contacts": s.showContactsTab = value; case "calls": s.showCallsTab = value; case "wideTabBar": s.wideTabBar = value; case "profileId": s.showProfileId = value; case "dc": s.showDc = value; case "regDate": s.showRegistrationDate = value; case "chatDate": s.showChatCreationDate = value; case "confirmCalls": s.confirmCalls = value; case "disableAds": s.disableAds = value; default: break }
    })
}
