...
func bgChatsSettingsController(context: AccountContext) -> ViewController {
    ...
    return bgController(context: context, title: "Чаты", entries: {
        ...
        switch key {
        case "reactions": return bgHiddenReactionsController(context: context)
        case "transcription": return bgTranscriptionController(context: context)
        case "camera": return bgRoundVideoCameraController(context: context)
        case "autoPauseMedia": return bgAutoPauseMediaController(context: context)
        default: return nil
        }
    })
}

private func bgRoundVideoCameraController(context: AccountContext) -> ViewController {
    let s = BGSimpleSettings.shared
    return bgController(context: context, title: "Камера в кружках", entries: {
    ...
    }, select: { s.roundVideoCamera = BGSimpleSettings.RoundVideoCamera(rawValue: Int($0) ?? 0) ?? .front })
}

private func bgAutoPauseMediaController(context: AccountContext) -> ViewController {
    let s = BGSimpleSettings.shared
    return bgController(context: context, title: "Авто пауза", entries: {
